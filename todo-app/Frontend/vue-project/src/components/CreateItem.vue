...
import { useCreateModalStore } from '../stores/CreateItemStore.ts';
...
const createNewItem = () => {
  ...
  const todo: todoUser = {
    id: Date.now(),
    title: newItem.value,
    comment: newComment.value || 'No comment',
    isComplete: false,
  };
  newItem.value = '';
  newComment.value = '';
...
<template>
  <Teleport to="body">
    ...
    >
      <div
        v-if="showModal"
        class="fixed inset-0 z-99 flex items-center justify-center bg-black/60"
